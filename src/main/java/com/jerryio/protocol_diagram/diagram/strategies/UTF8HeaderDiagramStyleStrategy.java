...
public class UTF8HeaderDiagramStyleStrategy extends UTF8DiagramStyleStrategy {
	...
	public void execute(Canvas canvas) {
		for (int i = 0; i < canvas.getWidth(); i++) {
			for (int j = 0; j < canvas.getHeight(); j++) {
				...
				if (j == 0) {
					if (ch == GridType.TOP_LEFT.toChar()) canvas.setChar(pos, this.getNonLeftChar());
					if (ch == GridType.TOP_RIGHT.toChar()) canvas.setChar(pos, this.getNonRightChar());
					if (ch == GridType.NON_TOP.toChar()) canvas.setChar(pos, this.getAllChar());
				} else {
