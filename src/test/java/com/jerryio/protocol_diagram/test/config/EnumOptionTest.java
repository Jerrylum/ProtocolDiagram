...
public class EnumOptionTest {
    ...
    public void testEnumOptionSetParameter(){
        ...
        assertEquals(eo.setValue(Parameter.parse(new CodePointBuffer("abc"))).message(), "Set \"test\" from \"bbb\" to \"abc\".");
        assertEquals(eo.setValue(Parameter.parse(new CodePointBuffer("5"))).success(), false);
        assertEquals(eo.setValue(Parameter.parse(new CodePointBuffer("5"))).message(), "The value \"5\" is not accepted.");
    }
}
