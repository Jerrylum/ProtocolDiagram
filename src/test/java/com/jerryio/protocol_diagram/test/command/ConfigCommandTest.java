...
public class ConfigCommandTest {
    ...
    public void testConfigCommandHandleSuccess() {
        ...
        assertEquals(cc.handle(CommandLine.parse(
                new CodePointBuffer("config bIt 128"))).success(), true);
        assertEquals(cc.handle(CommandLine.parse(
                new CodePointBuffer("config biT 32"))).success(), true);
        assertEquals(cc.handle(CommandLine.parse(
                new CodePointBuffer("CONFIG BIT 64"))).success(), true);
        assertEquals(cc.handle(CommandLine.parse(
                new CodePointBuffer("config diagram-style utf8-HEADER"))).success(), true);
        assertEquals(cc.handle(CommandLine.parse(
                new CodePointBuffer("config diagram-style UTF8-corner"))).success(), true);
        assertEquals(cc.handle(CommandLine.parse(
                new CodePointBuffer("config diagram-style ASCII"))).success(), true);
        assertEquals(cc.handle(CommandLine.parse(
                new CodePointBuffer("config DIAGRAM-style ascii-verbose"))).success(), true);
        assertEquals(cc.handle(CommandLine.parse(
                new CodePointBuffer("config diagram-STYLE utf8"))).success(), true);
        assertEquals(cc.handle(CommandLine.parse(
                new CodePointBuffer("config HEADER-style none"))).success(), true);
        assertEquals(cc.handle(CommandLine.parse(
                new CodePointBuffer("config header-STYLE FULL"))).success(), true);
        assertEquals(cc.handle(CommandLine.parse(
                new CodePointBuffer("config header-style trim"))).success(), true);
        assertEquals(cc.handle(CommandLine.parse(
                new CodePointBuffer("config left-space-placeholder true"))).success(), true);
        assertEquals(cc.handle(CommandLine.parse(
                new CodePointBuffer("config left-space-placeholder false"))).success(), true);
    }
    ...
    public void testConfigCommandHandleFail() {
        ...
        assertEquals(cc.handle(CommandLine.parse(new CodePointBuffer("config unknown 1"))).success(), false);
    }
}
